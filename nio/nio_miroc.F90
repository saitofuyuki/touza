...
!!!_  - default
  ...
  logical,save,public :: fix_cdate = .FALSE.
  logical,save,public :: fix_mdate = .FALSE.
...
!!!_  - set_fix_date - fix [CM]DATE ITEM as given (mainly for debug)
  subroutine set_fix_date(ierr, tag, idate)
    use TOUZA_Nio,only: set_default_date
    implicit none
    integer,         intent(out) :: ierr
    character(len=*),intent(in)  :: tag
    integer,optional,intent(in)  :: idate(*)
    ierr = 0
    if (index('Cc', tag(1:1)).gt.0) then
       if (present(idate)) then
          call set_default_date(ierr, cdate=idate)
          fix_cdate = .TRUE.
       else
          fix_cdate = .FALSE.
       endif
    else if (index('Mm', tag(1:1)).gt.0) then
       if (present(idate)) then
          call set_default_date(ierr, mdate=idate)
          fix_mdate = .TRUE.
       else
          fix_mdate = .FALSE.
       endif
...
subroutine GTZWRZ &
  ...
  use TOUZA_Nio_miroc,only:  fix_cdate, fix_mdate
...
# endif
  ...
  if (TIME .ge. TIME_PREV + LAZINESS) then
     ...
     idtv(4:6) = idtv(5:7)
     if (.not.fix_cdate) call put_item_date(jerr, hdefv, idtv(1:6), hi_CDATE)
     if (.not.fix_mdate) call put_item_date(jerr, hdefv, idtv(1:6), hi_MDATE)
     TIME_PREV = TIME
  endif
