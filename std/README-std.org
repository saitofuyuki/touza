...
**** ~find_next_sep()~
#+begin_src f90
  integer function find_next_sep(str, sep, pos, offset)
    character(len=*),intent(in) :: str
    character(len=*),intent(in) :: sep
    integer,optional,intent(in) :: pos
    integer,optional,intent(in) :: offset
#+end_src
Return position of =sep= occurence in =str= starting from =pos= or head.
Optional argument =offset= can change the index counting from the module
default value =0=,
which can be controled by ~init~ subroutine.
*** subroutines
**** ~init~
#+begin_src f90
...
#+begin_src f90
  subroutine trace_fine &
    ...
    integer,         intent(in),optional :: levv
#+end_src
**** ~msg~ - message subroutine with formatted scaler/arrays outputs
#+begin_src f90
  subroutine msg(fmt, vv, tag, u)
  subroutine msg(fmt, v,  tag, u)
  subroutine msg(txt,     tag, u)
  subroutine msg(txt,          u)
    ...
    integer,         intent(in),optional :: u
#+end_src
The first two interfaces output array ~vv~ or scaler ~v~ using format
~fmt~ to i/o unit ~u~.  Argument ~tag~ is inserted before the message,
to help searching.  Nothing is inserted If ~tag~ is blank.
The other two interfaces just output ~txt~ string to i/o unit ~u~.
If ~u~ is absent, public parameter ~unit_global~ is adopted as the
target unit.  Then if the target unit is ~unit_global~, the unit
configured by the initialization is adopted as the target unit.
Then if the target unit is the same as public paramet ~unit_star~,
then ~write~ statement is called with ~format=*~; if the target unit
is positive, it is used as it is; otherwise logging is skipped.
The tag is surrounded by =[= and =]= if it is present and non-blank.

Examples of =msg()= family with string arguments.
#+begin_src f90
...
: ('with format (s) ', A)
: [tag] ('with format (s) ', A)
: [tag] with format (s) abc
: [tag] with format (a)   a  b cd

Examples of =msg()= family with integer arguments.
#+begin_src f90
  call msg('(''with format (s) '', I0)',    354,          'tag')
  call msg('(''with format (a) '', 3I3.2)', (/4, 9, 16/), ' ')
#+end_src
#+RESULTS:
: [tag] with format (s) 354
: with format (a)  04 09 16

**** ~msg_grp~
#+begin_src f90
  subroutine msg_grp(fmt, vv, grp, mdl, u)
  subroutine msg_grp(fmt, v,  grp, mdl, u)
  subroutine msg_grp(txt,     grp, mdl, u)
    character(len=*),intent(in)          :: txt
    character(len=*),intent(in)          :: fmt
    <type>,          intent(in)          :: vv(:)
    <type>,          intent(in)          :: v
...
**** ~msg_mdl~
#+begin_src f90
  subroutine msg_mdl(fmt, vv, mdl, u)
  subroutine msg_mdl(fmt, v,  mdl, u)
  subroutine msg_mdl(txt,     mdl, u)
    character(len=*),intent(in)          :: fmt
    character(len=*),intent(in)          :: txt
    <type>,          intent(in)          :: vv(:)
    <type>,          intent(in)          :: v
...
**** ~msg_fun~
#+begin_src f90
  subroutine msg_fun(fmt, vv, mdl, fun, u)
  subroutine msg_fun(txt,     mdl, fun, u)
    character(len=*),intent(in)          :: txt
    character(len=*),intent(in)          :: fmt
...
#+end_src
**** ~banner~ - output banner (one-line title bar) string
#+begin_src f90
  subroutine banner &
...
#+end_src
Output one-lie title bar of string ~txt~ argument to io-unit ~u~ or
standard output.
The title is surrounded by letter(s) ~ch~ argument, or =+= by default.
Argument ~ww~ specifies the left and right margin width, (3 by default).
Argument ~indent~ specifies the amount of indent spaces (or 0).
Argument ~pat~ specifies the pattern of banner: if ~pat~ is
non-negative, the upper an lower surrounding lines are outputed in addition.
*** functions
**** ~get_logu()~
**** ~is_msglev()~
#+begin_src f90
...
**** ~is_error_match()~
#+begin_src f90
  logical function is_error_match (ierr, jcode, mdl)
    integer,intent(in)          :: ierr
...
*** description
Several kinds of build-time properties are diagnosed by this module.
*** subroutines
**** ~init~
**** ~diag~
**** ~finalize~
**** ~check_all~
#+begin_src f90
  subroutine check_all(ierr, ulog)
    integer,intent(out)         :: ierr
    integer,intent(in),optional :: ulog
#+end_src
Output build-time properties to io-unit ~ulog~ or standard output.
GCC, Intel, and/or NEC compiler properties, set by preprocessor macros
are outputed.
Version Control System VCS properties are output by ~check_touza~.

Example output, except for VCS properties as follows:
: [TOUZA/Std/bld] build info:__GFORTRAN__ 1
: [TOUZA/Std/bld] build info:__GNUC__ 12
: [TOUZA/Std/bld] build info:__GNUC_MINOR__ 2
: [TOUZA/Std/bld] build info:__GNUC_PATCHLEVEL__ 0
: [TOUZA/Std/bld] build info:__VERSION__ 12.2.0
: [TOUZA/Std/bld] build info:__OPTIMIZE__ 1
: [TOUZA/Std/bld] build info:__OPTIMIZE_SIZE__
: [TOUZA/Std/bld] build info:__NO_INLINE__
: [TOUZA/Std/bld] build info:__LP64__ 1
: [TOUZA/Std/bld] build info:__CHAR_BIT__ 8
: [TOUZA/Std/bld] build info:__BYTE_ORDER__ 1234
: [TOUZA/Std/bld] build info:__ORDER_LITTLE_ENDIAN__ 1234
: [TOUZA/Std/bld] build info:__ORDER_BIG_ENDIAN__ 4321
: [TOUZA/Std/bld] build info:__ORDER_PDP_ENDIAN__ 3412
: [TOUZA/Std/bld] build info:__NEC__
: [TOUZA/Std/bld] build info:__INTEL_COMPILER

**** ~check_touza~
#+begin_src f90
  subroutine check_touza(ierr, ulog)
    integer,intent(out)         :: ierr
    integer,intent(in),optional :: ulog
#+end_src
Output =TOUZA= VCS properties to io-unit ~ulog~
or standard output.

Example output is as follows:
: [TOUZA/Std/bld] build info:VCS_UUID 6d47ed9b7a64f36f5308ecdc4ea6d33df6884600
: [TOUZA/Std/bld] build info:VCS_NUM 3678
: [TOUZA/Std/bld] build info:VCS_DATA 2023-03-30T01:41:10Z
: [TOUZA/Std/bld] build info:VCS_BRANCH fix/ury-decodes
: [TOUZA/Std/bld] build info:VCS_TAG release/1.09.0
: [TOUZA/Std/bld] build info:VCS_TICK 136
: [TOUZA/Std/bld] build info:VCS_EXTRA
: [TOUZA/Std/bld] build info:VCS_ACTION_STAMP 2023-03-30T01:41:10Z!saitofuyuki@jamstec.go.jp
: [TOUZA/Std/bld] build info:VCS_FULL_HASH 9b5dcd8a318da8373d33233aea01aac1aaecd37f
: [TOUZA/Std/bld] build info:VCS_WC_MODIFIED 1
