...
- ~-~ :: insert the new empty-name coordinate to the specific rank.
  ~SHAPE=-~ insert the null coordinate at the first, and shift all the
  old coordinate by 1.

- ~-/<replace>[​/]~ :: insert the new coordinate with name =<replace>=
  ...
  which may not increase the ranks.

- ~<element>~ :: Extract =<element>=-th value along the specific
  coordinate (e.g., ~5~, non-negative integer).

- ~<range>~  :: Extract =<range>= values along the specific coordinate
  (e.g., ~3:5~, ~:7~, ~2:~).
